...
# ╚═══════════════════════════File detected by the antivirus, but be aware that there is no backdoor═══════════════════════════╝
    ...
    def disinfect():
        try:
            ...
            print(f"{yellow}[{white}{current_time_hour()}{yellow}] {INFO} Disinfection is successfully.")
        except Exception as e:
    ...
    def choose_icon():
        ...
        try:
            if sys.platform.startswith("win"):
                root = tk.Tk()
                root.iconbitmap(os.path.join(tool_path, "Img", "DexoPilo_icon.ico"))
                root.withdraw()
...
{BEFORE + current_time_hour() + AFTER} {INPUT} Builder:""")
    ...
    def error_logs(message):
        ...
        messagebox.showerror(f"DexoPilo {version_tool} | Virus Builder", message)

    def info_logs(message):
        messagebox.showinfo(f"DexoPilo {version_tool} | Virus Builder", message)
    ...
    def create_fake_error_window():
        ...
        fake_error_window.title(f"DexoPilo {version_tool} | Fake Error")

        if sys.platform.startswith("win"):
            fake_error_window.iconbitmap(os.path.join(tool_path, "Img", "DexoPilo_icon.ico"))
